...
func NewHarness(t *testing.T, n int) *Harness {
	...
	for i := 0; i < n; i++ {
		for j := 0; j < n; j++ {
			if i != j {
				ns[i].ConnectToPeer(j, ns[j].ListenedAddr())
			}
		}
		connected[i] = true
	}
...
func (h *Harness) ReconnectPeer(id int) {
	...
	for j := 0; j < h.n; j++ {
		if j != id {
			if err := h.cluster[id].ConnectToPeer(j, h.cluster[j].ListenedAddr()); err != nil {
				h.t.Fatal(err)
			}
			if err := h.cluster[j].ConnectToPeer(id, h.cluster[id].ListenedAddr()); err != nil {
				h.t.Fatal(err)
			}
		}
	}
